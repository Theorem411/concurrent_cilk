...
/*
  ...
  The protocol uses three shared pointers into the victim's deque: T
                                     ...
                                     __cilkrts_stack_frame *const loot_sf)
{
    ...
    /* Reverse the call stack to make a linked list ordered from parent
       ...
       the parent.  */
    do {
      // Detached, stolen, and the last frame cannot be stolen because in the case of the first two,
      // these frames are already being executed by another worker; in the case of the last frame, 
      // executing this frame would result in runtime exiting. 
    ...
    for (t_sf = __cilkrts_advance_frame(sf); t_sf; sf = t_sf, t_sf = __cilkrts_advance_frame(sf)) {
    ...
    __cilkrts_stack *sd)
{
  ...
  // The thief adopts the victim's team
  w->l->team = victim->l->team;

  // The frame must be null or this is a simulated steal
  // as steals are only done when the thief has no more work.
  // The only other option is this is a simulated steal in which case we may still have work. 
  ...
  // The victim is working from the tail (lower values) to the head (higher values)
  // incrementing the head by one pops a cilk_spawn continuation off the victim's dequeue. 
  victim->head = h + 1;

  // The victim's full frame is never null because their are two cases where a victim can have work:
  // 1. The victim is the first cilk worker which is created with a dummy full frame (no sf).
  // 2. The victim gained its work from a steal, at which point all stack frames will become full frames. 
  ...
  BEGIN_WITH_FRAME_LOCK(w, parent_ff) {
    ...
    if (WORKER_USER == victim->l->type && NULL == victim->l->last_full_frame) {
